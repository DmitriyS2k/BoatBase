...
struct BoatSettings {
    ...
    float tempAlarm     = 70.0f;   // °C — выше этого режем газ до tempLimitPwm во всех режимах (motors.cpp)
    int   tempLimitPwm  = 1650;    // потолок ПВМ при перегреве (motorsWrite применяет после trim, до финального constrain)
    ...
    float cruiseSteerRate = 15.0f; // °/с курса при полном отклонении сн4 (сн4 крутит цель, регулятор всегда в замкнутом контуре)
    ...
    int   gpsRateHz     = 2;      // частота обновления позиции GPS: 1/2/5 (см. gpsSetRate)
