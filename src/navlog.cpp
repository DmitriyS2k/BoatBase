        ...
        if (fillPending.length() == 0) {
            if (!fillConfigSent) {
                ...
                snprintf(cfgLine, sizeof(cfgLine),
                    ...
                    "bearingAlpha=%.2f navInterval=%d turnSlowFloor=%.2f motorMaxPwm=%d "
                    "navMode=%d losLookahead=%.1f cruiseSpeed=%d\n",
                    cfg.pidKp, cfg.pidKi, cfg.pidKd, cfg.pidCurve, cfg.maxDiff,
                    cfg.bearingAlpha, cfg.navInterval, cfg.turnSlowFloor, cfg.motorMaxPwm,
                    cfg.navMode, cfg.losLookahead, cfg.cruiseSpeed);
                fillPending = cfgLine;
                fillConfigSent = true;
